...
for node in range(nodesum, 0, -1): #各nodeについて
    ...
    for label in OldFronts:
        ...
        for i in patterns: #有りパターン
            ...
            for energysum in OldFronts[label]:
                new_energysum = energysum + energy_dic[i] #エネルギー合計を更新

                if (new_label in NewFronts) and (new_energysum in NewFronts[new_label]): #同じ条件のものがあれば
                    NewFronts[new_label][new_energysum] += OldFronts[label][energysum] #場合の数を追加
                else: #同じ条件のものがなかった場合
                    NewFronts.setdefault(new_label, {}).setdefault(new_energysum, OldFronts[label][energysum]) #新しくfrontを作る
                    frontsum += 1
    OldFronts = NewFronts
    NewFronts = {}
...
for label in OldFronts:
    for energysum in OldFronts[label]:
        sum += OldFronts[label][energysum] #配置パターン総数を求める
        if energysum in Energy:#同じ条件のものがあれば
            Energy[energysum] += OldFronts[label][energysum] #場合の数を追加
        else: #同じ条件のものがなかった場合
            Energy[energysum] = OldFronts[label][energysum]#新しく作る

elapsed_time = time.time() - start
f = open('newdpresult.txt', 'w')
